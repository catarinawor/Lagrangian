...
plot_logUtility <- function( M1 ,M2,M3, Msq1,Msq2,Msq3, sv=F, nome="",nations=F)
{
	cat("plot_logUtility")
	...
	for(y in 1:nsq){
		...
		utility_total_sq[y]<-sum(log(apply(Msq[[y]][[1]]$yYieldNat[101:160,],1,sum)+1))
		utility_total2_sq[y]<-sum(log(Msq[[y]][[1]]$yYieldNat[101:160,1]+1))*sum(log(Msq[[y]][[1]]$yYieldNat[101:160,2]+1))
		utility_nat1_sq[y]<-sum(log(Msq[[y]][[1]]$yYieldNat[101:160,1]+1))
		utility_nat2_sq[y]<-sum(log(Msq[[y]][[1]]$yYieldNat[101:160,2]+1))
	...
	dflu<-data.frame(slope_hcr=slope_hcr,intercept_hcr=intercept_hcr,
		...
		utility_nat2=utility_nat2,b40=B40_total)
	...
	dflusq<-data.frame(slope_hcr=slope_hcr_sq,intercept_hcr=intercept_hcr_sq,
		...
		utility_nat2=utility_nat2_sq,b40=B40_total_sq)
	...
	p<-p+geom_point(aes(y=utility_total2,x=b40,color=slope))
	p<-p+ ylab("log utility - Total") + xlab("% time Bt < 0.4 Bo")
	p<-p+ geom_text(aes(y=utility_total2[dfa$slope_hcr==999],x=b40[dfa$slope_hcr==999],label="10:40"),fontface = "bold",colour="black", position = position_nudge(y = -0.02))				
	p <- p  + theme_bw(16)
	p

	p1<-ggplot(dfa)
	p1<-p1+geom_line(aes(y=utility_nat1,x=b40,color=slope))
	p1<-p1+geom_point(aes(y=utility_nat1,x=b40,color=slope))
	p1<-p1+ ylab("log utility - Nation 1") + xlab("")
	p1<-p1+ geom_text(aes(y=utility_nat1[dfa$slope_hcr==999],x=b40[dfa$slope_hcr==999],label="10:40"),fontface = "bold",colour="black", position = position_nudge(y = -0.02))				
	p1 <- p1  + theme_bw(16)
	p1
	...
	p2<-p2+ geom_point(aes(y=utility_nat2,x=b40,color=slope))
	p2<-p2+ ylab("log utility - Nation 2") + xlab("")
	p2<-p2+ geom_text(aes(y=utility_nat2[dfa$slope_hcr==999],x=b40[dfa$slope_hcr==999],label="10:40"),fontface = "bold",colour="black", position = position_nudge(y = 0.02)	)		
	p2 <- p2  + theme_bw(16)
	p2

	plot_grid(p1,p2,p,nrow=3)
