...
{
	setwd(dir)
	assmt <- read.rep("hakelag.rep")
	par <- read.fit("hakelag")	
	C <- list(assmt=assmt,par=par)
	return( C );
}
	...
	tmp <- read.rep("hakelag.rep")
	assmt_nyr<- tmp$nyr	
	
	sims<-readOutput("/Users/catarinawor/Documents/iSCAM/examples/hakelag/DATA")
	setwd("/Users/catarinawor/Documents/Lagrangian/R/read_mse/result/")

	file.name <- paste("assess_yr",assmt_nyr,".Rdata",sep="")
	
	system(paste("mkdir",paste("run","iSCAM",seed,sep="")))
	setwd(paste("/Users/catarinawor/Documents/Lagrangian/R/read_mse/result/","run","iSCAM",seed,sep=""))
