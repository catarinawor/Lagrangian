...
#Author: Catarina Wor
#date: Oct 21th 2016
#Function to plot true depletion
...
plotOMBiomass <- function( M )
{
	...
	for(i in 1:n)
	{
		df <- data.frame(Model=as.factor(M[[i]]$seed),Depletion=M[[i]]$"ytB"/M[[i]]$"Bo",Year=(M[[i]]$rep_yr+1):M[[i]]$proj_yr)
		mdf <- rbind(mdf,df)
	}

	p <- ggplot(mdf,aes(x=Year,y=Depletion, color=Model)) + geom_line()
...
plotOMCIBiomass <- function( M )
{
	...
	cib <- data.frame(M[[1]]$"ytB"/M[[1]]$"Bo")

	for(i in 2:n){

		bio <- data.frame(M[[i]]$"ytB"/M[[i]]$"Bo")
		cib <- cbind(cib,bio)
	}
